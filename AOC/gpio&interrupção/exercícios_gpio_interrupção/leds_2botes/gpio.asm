@ Os LEDs se movem em direções opostas e reiniciam quando se encontram.
@ O botão + (PD2) aumenta a velocidade (reduz delay).
@ O botão - (PD3) diminui a velocidade (aumenta delay).
@ Os LEDs são deslocados usando LSL (Left Shift) e LSR (Right Shift).
...
; Definições de pinos
; ==========================
.equ BOTAO_AUMENTAR = PD2   ; Botão de aumentar velocidade (conectado ao pino PD2)
.equ BOTAO_DIMINUIR = PD3   ; Botão de diminuir velocidade (conectado ao pino PD3)
...
; ==========================
setup:
    ldi AUX, 0xFF         ; Carrega 0xFF (todos os bits em 1)
    out DDRB, AUX         ; Configura PB0-PB7 como saída para os LEDs

    ldi AUX, 0x00         ; Carrega 0x00 (todos os bits em 0)
...
; ==========================
main:
    clr AUX               ; Limpa o registrador auxiliar (zerar bits)

    ; ==============================
    ; Define o LED da esquerda (PB0 → PB7)
    ; ==============================
    ldi R20, 1            ; Carrega o valor 1 (0b00000001) no R20
    mov R21, INDEX_ESQ    ; Copia INDEX_ESQ para R21 (quantidade de deslocamentos)
loop_shift_esq:
...
led_esq_done:
    ...
    mov R21, INDEX_DIR    ; Copia INDEX_DIR para R21 (quantidade de deslocamentos)
loop_shift_dir:
