...
int fila_chegada[NUM_CARROS]; // ordem de chegada dos carros

void *carro(void *arg) {
    int id = (long) arg;
    int cancela = rand() % NUM_CANCELAS;

    // Salva a ordem de chegada global
    ...
    sleep(5); // leva 5 segundos para passar
    pthread_mutex_unlock(&mutex_cancelas[cancela]);

    // Avança para o próximo carro da fila
    pthread_mutex_lock(&mutex_ordem);
    ordem_atual++;
    pthread_mutex_unlock(&mutex_ordem);

    // Fica estacionado exatamente 10 segundos
...
// Compilar com gcc -Wall desafio1.c -o desafio1 -lpthread