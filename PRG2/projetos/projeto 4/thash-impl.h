...
namespace prg2 {
    ...
    // parâmetros: endereço da área de memória, qtds bytes, tamanho da tabela
...
//        return soma % N;
    ...
    void thash_expande (thash<K, V> & tab) {
        ...
        for (auto & lista: tab.tabela) {
            ...
            for (auto & par: lista) {
                ...
                // (agora com nova.size() para adaptar o resultado a nova tabela)
    ...
    void thash_adiciona(thash<K, V> &tab, const K &chave, const V &valor) {
        ...
        // tem q ter um tipo de função p cada tipo de chave
    ...
    V thash_obtem(thash<K, V> &tab, const K &chave) {
        ...
        throw std::invalid_argument("posicao nao esta ocupada");
    }
    ...
    void thash_remove(thash<K, V> &tab, const K &chave) {
        ...
        throw std::invalid_argument("chave nao existe");
